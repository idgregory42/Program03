...
int main()
{
   ...
   do
   {
	   ...
	   if(choice == 1)
	   {
			...
			cds_a = RadixSort<CD>::radixSort(cds, size, num_chars, true, &CD::getRadixChar);  //sorts dem cds
			displayDem(cds_a, size);
			a->displayString();
	   ...
	   {
			cds_a = RadixSort<CD>::radixSort(cds, size, num_chars, false, &CD::getRadixChar);  //sorts dem cds
			displayDem(cds_a, size);
			d->displayString();
